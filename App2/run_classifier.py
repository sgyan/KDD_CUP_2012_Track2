...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook({"logits": logits}, every_n_iter=1)
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def input_fn_builder_predict(features, seq_length, is_training, drop_remainder):
  ...
  if FLAGS.do_predict:
      ...
      with tf.gfile.GFile(output_predict_file, "w") as writer:
          ...
          for i,result1 in enumerate(result):
            writer.write('{}\t{}\n'.format(i, str(result1['softmax_all'][1])))
  ...
  if FLAGS.do_eval:
    ...
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      ...
      for key in sorted(result.keys()):
        if key == 'softmax_all':
            pass
            #tf.logging.info(key, str(result[key]))
        else:
            tf.logging.info("  %s = %s", key, str(result[key]))
            writer.write("%s = %s\n" % (key, str(result[key])))
