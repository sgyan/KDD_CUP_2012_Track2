...
flags.DEFINE_string(
    ...
    "")
flags.DEFINE_integer("train_line_cnt", 0,
                     "input data instance num.")
flags.DEFINE_integer("top_k_num", 60,
                     "the num will be extraction")
...
class MrpcProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      label = tokenization.convert_to_list(line[0],float) #Teacher Predict Emb40
...
def convert_examples_to_features(examples, label_list, max_seq_length,
  ...
  for (ex_index, example) in enumerate(examples):
    if ex_index < last_checkpoint:
        continue
    if save_checkpoint and ex_index % (FLAGS.save_checkpoints_steps * FLAGS.train_batch_size) == 0:
      with tf.gfile.GFile(data_checkpoint_file, "w") as writer:
        writer.write("%d" % (ex_index))
    tokens_a, terms_a, term_segments_ids_a = tokenizer.tokenize(example.text_a.replace("[SEP]"," ")) # terms_a is string list
    ...
    if len(term_segments_ids_a) > max_seq_length - 1:
      term_segments_ids_a = term_segments_ids_a[0:(max_seq_length - 1)]
    if len(terms_a) > max_seq_length - 1:
      terms_a = terms_a[0:(max_seq_length - 1)]

    # pad up to the sequence length-1.
    pad_id = term_segments_ids_a[-1]+1
    while len(term_segments_ids_a) < max_seq_length-1:
      term_segments_ids_a.append(pad_id)

    # pad up to the sequence length-1.
    while len(terms_a) < max_seq_length-1:
      terms_a.append("")
    
    # if term_segments_ids_a[-1] < FLAGS.top_k_num:
    #   tf.logging.info("Warning:terms_a: [%s]" % (" ".join(terms_a)))
    #   tf.logging.info("Warning:term_segments_ids_a: [%s]" % (" ".join([str(x) for x in term_segments_ids_a])))
    #   continue
    ...
    if ex_index < 5:
      ...
      tf.logging.info("terms_a: [%s]" % (" ".join(terms_a)))
      tf.logging.info("term_segments_ids_a: [%s]" % (" ".join([str(x) for x in term_segments_ids_a])))
    ...
    yield guid, input_ids, lm_input_ids, input_mask, segment_ids, masked_lm_positions, masked_lm_labels, masked_lm_weights, pos_id, perc, weight, pos_bias, example.label, term_segments_ids_a, terms_a
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 num_labels, pos_ids, percs, weight, pos_bias, use_one_hot_embeddings,term_segments_ids_a=None,terms_a=None):
  ...
  attprobs = model.get_attprobs_output() # [batch_size, seq_length-1]


 # because tf.shape(attprobs)[0] get a tensor ,num_or_size_splits must be a specific num. so change the bz to 1
  # # attprobs_split = tf.split(attprobs, num_or_size_splits=tf.shape(attprobs)[0], axis=0)
  # # term_segments_ids_a_split = tf.split(term_segments_ids_a, num_or_size_splits=tf.shape(attprobs)[0], axis=0) #term_segments_ids_a [train_batch_size,FLAGS.max_seq_length-1]
  # # result_indices = []
  # # for idx,prob in enumerate(attprobs_split):
  # #   term_avg_att_score = tf.math.segment_mean(tf.reshape(prob,[FLAGS.max_seq_length-1]),tf.reshape(term_segments_ids_a_split[idx],[FLAGS.max_seq_length-1]))
  # #   # term_avg_att_score_dim0 = term_avg_att_score.shape.as_list()[0]
  # #   # if term_avg_att_score_dim0 < FLAGS.top_k_num:
  # #   #   tf.logging.info("Warning Real Att Score Dim: [%s]", str(term_avg_att_score_dim0))
  # #   # assert term_avg_att_score_dim0 >= FLAGS.top_k_num
  # #   _, indices = tf.nn.top_k(term_avg_att_score,k=FLAGS.top_k_num,sorted=False)
  # #   result_indices.append(tf.reshape(indices,[1,FLAGS.top_k_num]))
  # # result_indices = tf.concat(result_indices, 0)
  # term_avg_att_score = tf.math.segment_mean(tf.reshape(attprobs,[FLAGS.max_seq_length-1]),tf.reshape(term_segments_ids_a,[FLAGS.max_seq_length-1]))
  # _, indices = tf.nn.top_k(term_avg_att_score,k=FLAGS.top_k_num,sorted=False)
  sub_train_vars = []
  hidden_size = output_layer.shape[-1].value
  count_hidden_size=40
  ...
  with tf.variable_scope("loss", reuse=tf.AUTO_REUSE):
    ...
    return bert_logits, sub_train_vars, model, attprobs, terms_a, term_segments_ids_a
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    term_segments_ids_a = features["term_segments_ids_a"]
    terms_a = features["terms_a"]
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      (logits, sub_train_vars, model, attprobs, terms_a, term_segments_ids_a) = create_model(
      ...
      if FLAGS.multi_task:
        (lm_logits, lm_sub_train_vars, lm_model, attprobs, terms_a, term_segments_ids_a) = create_model(
      ...
      auc = tf.metrics.auc(label_ids, pred)
      eval_metrics = {
                        "eval_loss": loss,
                        "eval_auc": auc
                     }

      logging_hook = tf.train.LoggingTensorHook({"pred": pred}, every_n_iter=1000)
      output_spec = tf.estimator.EstimatorSpec(
          mode=mode,
          loss=total_loss,
      ...
      (logits, sub_train_vars, model, attprobs, terms_a, term_segments_ids_a) = create_model(
          bert_config, is_training, input_ids, input_mask, segment_ids,
          num_labels, pos_ids, percs, weight, pos_bias, use_one_hot_embeddings,term_segments_ids_a,terms_a)
      ...
      output_spec = tf.estimator.EstimatorSpec(
          ...
          predictions={
            "pred":attprobs,
            "terms_a":terms_a,
            "term_segments_ids_a":term_segments_ids_a,
            "guid": guids
          }
      )
    else:
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  def input_fn(params):
    def _decode(guids, input_ids, lm_input_ids, input_mask, segment_ids, masked_lm_positions, masked_lm_labels, masked_lm_weights, pos_ids, percs, weight, pos_bias, label_ids, term_segments_ids_a, terms_a):
      return {"guids": guids, "input_ids":input_ids, "lm_input_ids":lm_input_ids, "input_mask":input_mask, "segment_ids":segment_ids, "pos_ids": pos_ids, "percs": percs, "weight":weight, "pos_bias":pos_bias,
           "masked_lm_positions": masked_lm_positions, "masked_lm_labels": masked_lm_labels, "masked_lm_weights": masked_lm_weights,"term_segments_ids_a":term_segments_ids_a,"terms_a":terms_a}, label_ids

    batch_size = FLAGS.train_batch_size
  
    d = tf.data.Dataset.from_generator(
        features,
        (tf.string, tf.int32, tf.int32, tf.int32, tf.int32, tf.int32, tf.int32, tf.float32, tf.int32, tf.float32, tf.float32, tf.float32, tf.float32,tf.int32,tf.string),
        (
          tf.TensorShape([]),
...
def main(_):
  ...
  session_config.gpu_options.visible_device_list = str("0") #"0" 
  ...
  if FLAGS.do_predict:
    ...
    if tf.gfile.Exists(data_checkpoint_file):
      with tf.gfile.GFile(data_checkpoint_file, "r") as fin:
        ...
        last_checkpoint = max(0, last_checkpoint - (FLAGS.train_batch_size) * 100)
    ...
    with tf.gfile.GFile(output_predict_file, "a") as writer:
      ...
      for i, result1 in enumerate(result):
        result_indices =[str(res) for res in result1["term_segments_ids_a"]]
        terms_a = [str(res.decode("utf-8")) for res in result1["terms_a"]]
        writer.write("%s\t%s\t%s\t%s\n" %(str(result1["guid"]), ' '.join(terms_a),';'.join(result_indices),';'.join("%.6f" % res for res in result1["pred"])))
        if i % 50000 == 0:
            tf.logging.info("Predict %s\t%s\t%s\t%s", str(result1["guid"]), ' '.join(terms_a),';'.join(result_indices),';'.join("%.6f" % res for res in result1["pred"]))
