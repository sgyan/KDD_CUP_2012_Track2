...
if __name__ == "__main__":
    ...
    def textrank_processor(input_file_name, examples):
      print(f"process {os.getpid()} is processing {input_file_name} ")
      nlp = spacy.load("en_core_web_lg")
      # add PyTextRank to the spaCy pipeline
      nlp.add_pipe("textrank")

      output_predict_file = os.path.join(args.output_dir, "output/" + input_file_name)
      with open(output_predict_file, "a") as writer:
        for idx, example in enumerate(examples):
          ...
          doc = nlp(" ".join(example.text_a.split("[SEP]")))
          ...
          #     print(phrase.chunks)
          writer.write("%s\t%s\n" % (example.guid, ' '.join(result[:50])))
          if idx % 50000 == 0:
              print("Predict [%s][%s]->[%s]", str(os.getpid()), example.guid, ' '.join(result[:50]))
    def task(filename):
      examples = processor.get_test_examples(filename)
      textrank_processor(filename, examples)
      return
    from pathos.multiprocessing import ProcessingPool as Pool
    
    # file_dir = os.path.dirname(args.data_dir)
    os.chdir(args.output_dir) # "split -n l/50 $DATA_DIR" Data saved in output_dir
    # os.system("mkdir -p hdf5")
    if not os.path.exists("./output/"):
        os.makedirs("./output/")
    input_files = [i for i in os.listdir(".") if i.startswith("x")]
    input_files = sorted(input_files)
    print(f"tsv files are {input_files}")
    process_pool = Pool(50)
    process_pool.map(task, input_files)
    print(f"convert all tsv file {input_files} to textrank done")
