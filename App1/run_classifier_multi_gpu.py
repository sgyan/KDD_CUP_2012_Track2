...
def convert_examples_to_features(examples, label_list, max_seq_length,
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    yield guid, input_ids, lm_input_ids, input_mask, segment_ids, masked_lm_positions, masked_lm_labels, masked_lm_weights, pos_id, perc, weight, pos_bias, label_id
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss", reuse=tf.AUTO_REUSE):
    ...
    return logits, sub_train_vars, model
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  def input_fn(params):
    ...
    d = tf.data.Dataset.from_generator(
        features,
        (tf.string, tf.int32, tf.int32, tf.int32, tf.int32, tf.int32, tf.int32, tf.float32, tf.int32, tf.float32, tf.float32, tf.float32, tf.int32),
        (
          tf.TensorShape([]),
