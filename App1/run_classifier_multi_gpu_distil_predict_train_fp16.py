...
class MrpcProcessor(DataProcessor):
  ...
  def _create_test_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_a = tokenization.convert_to_unicode(line[3])
      text_b = tokenization.convert_to_unicode(line[4])
      label = [float(0)]*29 #tokenization.convert_to_unicode(line[0]) # can't use this even data contains label
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "a") as writer:
      ...
      for i, result1 in enumerate(result):
        writer.write("%s\t%s\n" % (str(result1["guid"]), ';'.join("%.16f" % res for res in result1["pred"])))
        if i % 50000 == 0:
            tf.logging.info("Predict [%s]->[%s]", str(result1["guid"]), ';'.join("%.16f" % res for res in result1["pred"]))
